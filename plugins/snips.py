...
from blade._misc import sudoers
from blade.dB.snips_db import add_snip, get_snips, list_snip, rem_snip
...
async def an(e):
    ...
    blade_x_userbot_cmd.add_handler(add_snips, events.NewMessage())


@blade_cmd(pattern="remsnip( (.*)|$)")
async def rs(e):
    wrd = (e.pattern_match.group(1).strip()).lower()
    if not wrd:
    ...
    for z in xx:
        if k := get_snips(z):
            ...
            if rep:
                ...
                await blade_x_userbot_cmd.send_message(e.chat_id, msg, file=media)

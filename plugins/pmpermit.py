...
if udB.get_key("PMSETTING"):
    if udB.get_key("AUTOAPPROVE"):
        ...
        async def autoappr(e):
            ...
            try:
                await blade_x_userbot_cmd.edit_folder(miss.id, folder=0)
    ...
    async def permitpm(event):
        ...
        if not keym.contains(user.id) and event.text != UND:
            if Paltan("MOVE_ARCHIVE"):
                try:
                    await blade_x_userbot_cmd.edit_folder(user.id, folder=1)
            ...
            if user.id in LASTMSG:
                ...
                if event.text != prevmsg:
                    ...
                    if inline_pm:
                        results = await blade_x_userbot_cmd.inline_query(
                        ...
                        _to_delete[user.id] = await blade_x_userbot_cmd.send_file(
                            user.id,
                            PMPIC,
                            caption=message_,
                        )
                    else:
                        _to_delete[user.id] = await blade_x_userbot_cmd.send_message(
                            user.id, message_
                        )
            ...
            if COUNT_PM[user.id] >= WARNS:
                ...
                await blade_x_userbot_cmd(BlockRequest(user.id))
                await blade_x_userbot_cmd(ReportSpamRequest(peer=user.id))
                await asst.edit_message(
                    udB.get_key("LOG_CHANNEL"),
                    _not_approved[user.id],
    ...
    @blade_cmd(pattern="(start|stop|clear)archive$", fullsudo=True)
    async def _(e):
    ...
    @blade_cmd(pattern="(a|approve)(?: |$)", fullsudo=True)
    ...
    @blade_cmd(pattern="(da|disapprove)(?: |$)", fullsudo=True)
...
@blade_cmd(pattern="block( (.*)|$)", fullsudo=True)
...
@blade_cmd(pattern="unblock( (.*)|$)", fullsudo=True)
...
@blade_cmd(pattern="listapproved$", owner=True)
async def list_approved(event):
    ...
    for i in all:
        try:
            name = get_display_name(await blade_x_userbot_cmd.get_entity(i))
        except BaseException:
            name = ""
...
@callback(
    ...
    from_users=[blade_x_userbot_cmd.uid],
)
async def apr_in(event):
    ...
    if not keym.contains(uid):
        keym.add(uid)
        try:
            await blade_x_userbot_cmd.edit_folder(uid, folder=0)
        except BaseException:
            pass
        try:
            user = await blade_x_userbot_cmd.get_entity(uid)
        except BaseException:
...
async def blck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    try:
        await blade_x_userbot_cmd(BlockRequest(uid))
    except BaseException:
        pass
    try:
        user = await blade_x_userbot_cmd.get_entity(uid)
...
async def unblck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    try:
        await blade_x_userbot_cmd(UnblockRequest(uid))
    except BaseException:
        pass
    try:
        user = await blade_x_userbot_cmd.get_entity(uid)
...
async def ytfuxist(e):
    try:
        ...
        await blade_x_userbot_cmd.delete_messages(e.chat_id, e.id)
...
@callback(re.compile("admin_only(.*)"), from_users=[blade_x_userbot_cmd.uid])

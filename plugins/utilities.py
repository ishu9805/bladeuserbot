...
from blade._misc._assistant import asst_cmd
from blade.dB.gban_mute_db import is_gbanned
from blade.fns.tools import get_chat_and_msgid
...
from blade.fns.info import get_chat_info
...
@blade_cmd(pattern="kickme$", fullsudo=True)
async def leave(ult):
...
@blade_cmd(pattern="paste( (.*)|$)", manager=True, allow_all=True)
async def _(event):
    try:
...
@blade_cmd(pattern="suggest( (.*)|$)", manager=True)

...
from blade.dB import stickers
from blade.dB.echo_db import check_echo
from blade.dB.forcesub_db import get_forcesetting
from blade.dB.gban_mute_db import is_gbanned
from blade.dB.greetings_db import get_goodbye, get_welcome, must_thank
from blade.dB.nsfw_db import is_profan
from blade.fns.helper import inline_mention
from blade.fns.tools import async_searcher, create_tl_btn, get_chatbot_reply

try:
    from ProfanityDetector import detector
except ImportError:
    detector = None
from . import LOG_CHANNEL, LOGS, asst, get_string, types, udB, blade_x_userbot_cmd
from ._inline import something


@blade_x_userbot_cmd.on(events.ChatAction())
...
async def DummyHandler(ult):
    ...
    if (
        ...
        if not user.bot:
            ...
            try:
                await blade_x_userbot_cmd(GetParticipantRequest(int(joinchat), user.id))
            except UserNotParticipantError:
                await blade_x_userbot_cmd.edit_permissions(
                    ult.chat_id, user.id, send_messages=False
                )
                res = await blade_x_userbot_cmd.inline_query(
...
@blade_x_userbot_cmd.on(events.NewMessage(incoming=True))
...
@blade_x_userbot_cmd.on(events.Raw(types.UpdateUserName))

...
async def set_afk(event):
    ...
    if text and media:
        if "sticker" in media_type:
            msg1 = await blade_x_userbot_bot.send_file(event.chat_id, file=media)
            msg2 = await blade_x_userbot_bot.send_message(
                event.chat_id, get_string("afk_5").format(text)
            )
        else:
            msg1 = await blade_x_userbot_bot.send_message(
                event.chat_id, get_string("afk_5").format(text), file=media
            )
    elif media:
        if "sticker" in media_type:
            msg1 = await blade_x_userbot_bot.send_file(event.chat_id, file=media)
            msg2 = await blade_x_userbot_bot.send_message(event.chat_id, get_string("afk_6"))
        else:
            msg1 = await blade_x_userbot_bot.send_message(

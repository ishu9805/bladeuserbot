...
from blade import _ignore_eval
...
@blade_cmd(pattern="bash", fullsudo=True, only_devs=True)
async def _(event):
...
bot = ultroid = blade_x_userbot_cmd
...
@blade_cmd(pattern="eval", fullsudo=True, only_devs=True)
async def _(event):
    try:
        cmd = event.text.split(maxsplit=1)[1]
    except IndexError:
    ...
    if any(item in cmd for item in KEEP_SAFE().All) and (
        not (event.out or event.sender_id == blade_x_userbot_cmd.uid)
    ):
...
@blade_cmd(pattern="cpp", only_devs=True)
async def doie(e):

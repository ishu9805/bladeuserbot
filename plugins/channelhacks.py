...
from . import LOGS, asst, eor, events, get_string, udB, blade_x_userbot_cmd, blade_cmd
...
async def source(e):
    ...
    if not SourceM.contains(y):
        ...
        blade_x_userbot_cmd.add_handler(autopost_func, events.NewMessage())
    else:
        await e.eor(get_string("cha_3"))


@blade_cmd(pattern="dsource( (.*)|$)")
async def dd(event):
...
@blade_cmd(pattern="listdest")
async def list_all(event):
    blade_x_userbot_cmd = event.client
    x = await event.eor(get_string("com_1"))
    channels = DestiM.get()
    num = len(channels)
    if not num:
    ...
    for channel in channels:
        name = ""
        try:
            name = get_display_name(await blade_x_userbot_cmd.get_entity(int(channel)))
        except BaseException:
            name = ""
        msg += f"\n=> **{name}** [`{channel}`]"
    msg += f"\nTotal {num} channels."
    if len(msg) > 4096:
        ...
        with io.BytesIO(str.encode(MSG)) as out_file:
            out_file.name = "channels.txt"
            await blade_x_userbot_cmd.send_file(
                event.chat_id,
                out_file,
                force_document=True,
                allow_cache=False,

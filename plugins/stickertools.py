...
async def hehe(args):
    blade_x_userbot_cmd = args.client
    xx = await args.eor(get_string("com_1"))
    user = blade_x_userbot_cmd.me
    ...
    if message.photo:
        photo = io.BytesIO()
        photo = await blade_x_userbot_cmd.download_media(message.photo, photo)
    elif message.file and "image" in message.file.mime_type.split("/"):
        photo = io.BytesIO()
        await blade_x_userbot_cmd.download_file(message.media.document, photo)
        if (
        ...
        await blade_x_userbot_cmd.download_file(
    ...
    if photo:
        ...
        if not emoji:
            emoji = "🏵"
        if len(splat) == 3:
            pack = splat[2]  # User sent blade_x_userbot_cmdh
            emoji = splat[1]
        elif len(splat) == 2:
        ...
        if (
            ...
            async with blade_x_userbot_cmd.conversation("@Stickers") as conv:
                try:
                    await conv.send_message("/addsticker")
                except YouBlockedUserError:
                    LOGS.info("Unblocking @Stickers for kang...")
                    await blade_x_userbot_cmd(functions.contacts.UnblockRequest("stickers"))
                    await conv.send_message("/addsticker")
                ...
                await conv.get_response()
                await blade_x_userbot_cmd.send_read_acknowledge(conv.chat_id)
        else:

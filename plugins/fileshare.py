...
from blade.dB.filestore_db import del_stored, get_stored_msg, list_all_stored_msgs
from blade.fns.tools import get_file_link

from . import HNDLR, asst, get_string, in_pattern, udB, blade_x_userbot_cmd, blade_cmd


@blade_cmd(pattern="store$")
...
async def _(event):
    ...
    del_stored(match[1])
    await blade_x_userbot_cmd.delete_messages(udB.get_key("LOG_CHANNEL"), int(msg_id))
    await event.eor("__Deleted__")


@blade_cmd("liststored$")

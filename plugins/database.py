...
from . import Paltan, eor, get_string, udB, blade_cmd


@blade_cmd(pattern="setdb( (.*)|$)", fullsudo=True)
async def _(ult):
...
@blade_cmd(pattern="deldb( (.*)|$)", fullsudo=True)
async def _(ult):
...
@blade_cmd(pattern="rendb( (.*)|$)", fullsudo=True)
async def _(ult):
    match = ult.pattern_match.group(1).strip()
    if not match:

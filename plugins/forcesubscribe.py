...
from blade.dB.forcesub_db import add_forcesub, get_forcesetting, rem_forcesub
...
@blade_cmd(pattern="fsub( (.*)|$)", admins_only=True, groups_only=True)
async def addfor(e):
    ...
    blade_x_userbot_cmd.add_handler(force_sub, events.NewMessage(incoming=True))


@blade_cmd(pattern="remfsub$")
async def remor(e):
...
async def fcall(e):
    match = e.pattern_match.group(1).strip()
    spli = match.split("_")
    user = await blade_x_userbot_cmd.get_entity(int(spli[0]))
    cl = await blade_x_userbot_cmd.get_entity(int(spli[1]))
    text = f"Hi {inline_mention(user)}, You Need to Join"
    text += f" {cl.title} in order to Chat in this Group."
    el = (
        f"https://t.me/{cl.username}"
        if cl.username
        else (await blade_x_userbot_cmd(ExportChatInviteRequest(cl))).link
    )
...
async def diesoon(e):
    ...
    try:
        values = await blade_x_userbot_cmd(GetParticipantRequest(int(spli[1]), int(spli[0])))
    ...
    await blade_x_userbot_cmd.edit_permissions(
...
async def force_sub(ult):
    ...
    if count in range(2, 11):
        return
    try:
        await blade_x_userbot_cmd.get_permissions(int(joinchat), user.id)
        return
    except UserNotParticipantError:
        pass
    if isinstance(user, Channel):
        try:
            ...
            LOGS.exception(er)
    try:
        await blade_x_userbot_cmd.edit_permissions(ult.chat_id, user.id, send_messages=False)
    except ChatAdminRequiredError:
        return
    except Exception as e:
        await ult.delete()
        LOGS.info(e)
    res = await blade_x_userbot_cmd.inline_query(asst.me.username, f"fsub {user.id}_{joinchat}")

...
from . import get_string, udB, blade_x_userbot_cmd, blade_cmd

keym = KeyManager("NIGHT_CHATS", cast=list)


@blade_cmd(pattern="nmtime( (.*)|$)")
...
@blade_cmd(pattern="addnm( (.*)|$)")
async def add_grp(e):
    if pat := e.pattern_match.group(1).strip():
        try:
            keym.add((await blade_x_userbot_cmd.get_entity(pat)).id)
...
@blade_cmd(pattern="remnm( (.*)|$)")
async def r_em_grp(e):
    if pat := e.pattern_match.group(1).strip():
        try:
            keym.remove((await blade_x_userbot_cmd.get_entity(pat)).id)
...
@blade_cmd(pattern="listnm$")
async def rem_grp(e):
    chats = keym.get()
    name = "NightMode Groups Are-:\n\n"
    for x in chats:
        try:
            ok = await blade_x_userbot_cmd.get_entity(x)
...
async def open_grp():
    for chat in keym.get():
        try:
            ...
            await blade_x_userbot_cmd.send_message(chat, "**NightMode Off**\n\nGroup Opened 🥳.")

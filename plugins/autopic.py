...
from . import LOGS, get_help, get_string, udB, blade_x_userbot_cmd, blade_cmd
...
if search := udB.get_key("AUTOPIC"):
    ...
    async def autopic_func():
        ...
        file = await blade_x_userbot_cmd.upload_file(filee)
        await blade_x_userbot_cmd(UploadProfilePhotoRequest(file))
        os.remove(filee)

...
@blade_cmd(pattern="ping$", chats=[], type=["official", "assistant"])
async def _(event):
    start = time.time()
...
async def restartbt(ult):
    ...
    if len(sys.argv) > 1:
        ...
        os.execl(sys.executable, sys.executable, "-m", "blade")
...
async def _(e):
    ...
    if e.pattern_match.group(1).strip() and (
        ...
        os.execl(sys.executable, "python3", "-m", "blade")
        # return
    m = await updater()
    ...
    if m:
        ...
        await xx.edit(
            f'<code>Your BOT is </code><strong>up-to-date</strong><code> with </code><strong><a href="https://github.com/ishu9805/bladeuserbot/tree/{branch}">[{branch}]</a></strong>',
            parse_mode="html",
            link_preview=False,
        )

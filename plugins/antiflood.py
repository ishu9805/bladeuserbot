...
from . import Button, Paltan, asst, callback, eod, get_string, blade_x_userbot_cmd, blade_cmd
...
async def unmuting(e):
    ...
    user = int(ino[0])
    chat = int(ino[1])
    user_name = (await blade_x_userbot_cmd.get_entity(user)).first_name
    chat_title = (await blade_x_userbot_cmd.get_entity(chat)).title
    await blade_x_userbot_cmd.edit_permissions(chat, user, send_messages=True)
    await e.edit(

...
from blade.dB.gban_mute_db import (
    gban,
    gmute,
    is_gbanned,
    is_gmuted,
...
@blade_cmd(pattern="gpromote( (.*)|$)", fullsudo=True)
async def _(e):
    x = e.pattern_match.group(1).strip()
    blade_x_userbot_cmd = e.client
    if not x:
...
@blade_cmd(pattern="gdemote( (.*)|$)", fullsudo=True)
async def _(e):
    x = e.pattern_match.group(1).strip()
    blade_x_userbot_cmd = e.client
    if not x:
    ...
    if user:
        ...
        if user.isdigit():
            user = int(user)
        try:
            name = await blade_x_userbot_cmd.get_entity(user)
        except BaseException:
            return await e.eor(f"`No User Found Regarding {user}`", time=5)
        ...
        rank = "Not AdMin"
        c = 0
        async for x in blade_x_userbot_cmd.iter_dialogs():
            if (
                "group" in key.lower()
                and x.is_group
                or "group" not in key.lower()
...
@blade_cmd(pattern="ungban( (.*)|$)", fullsudo=True)
async def _(e):
...
@blade_cmd(pattern="gban( (.*)|$)", fullsudo=True)
async def _(e):
    ...
    try:
        ...
        name = str(userid)
    chats = 0
    if userid == blade_x_userbot_cmd.uid:
...
@blade_cmd(pattern="g(admin|)cast( (.*)|$)", fullsudo=True)
...
@blade_cmd(pattern="gucast( (.*)|$)", fullsudo=True)
...
@blade_cmd(pattern="gkick( (.*)|$)", fullsudo=True)
async def gkick(e):
...
@blade_cmd(pattern="gmute( (.*)|$)", fullsudo=True)
async def _(e):
...
@blade_cmd(pattern="ungmute( (.*)|$)", fullsudo=True)
async def _(e):
...
async def list_gengbanned(event):
    ...
    if len(gbanned_users) > 4096:
        ...
        await x.reply(
            file="gbanned.txt",
            message=f"List of users GBanned by {inline_mention(blade_x_userbot_cmd.me)}",
        )
...
@blade_cmd(pattern="gblacklist$")
async def blacklist_(event):
    await gblacker(event, "add")


@blade_cmd(pattern="ungblacklist$")

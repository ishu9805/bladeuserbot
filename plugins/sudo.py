...
from . import get_string, inline_mention, udB, blade_x_userbot_cmd, blade_cmd


@blade_cmd(pattern="addsudo( (.*)|$)", fullsudo=True)
async def _(ult):
...
@blade_cmd(pattern="delsudo( (.*)|$)", fullsudo=True)
async def _(ult):
    inputs = ult.pattern_match.group(1).strip()
    if ult.reply_to_msg_id:
        replied_to = await ult.get_reply_message()
